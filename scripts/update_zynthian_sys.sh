...
if [ -z "$NO_ZYNTHIAN_UPDATE" ]; then
	...
	# Configure RTC for 2 main boards
...
#--------------------------------------
if [[ $ZYNTHIAN_KIT_VERSION == Z2* ]]; then
	# If device is Z2, change extro video to miko
...
if [ -z "$NO_ZYNTHIAN_UPDATE" ]; then
	...
	cp -a $ZYNTHIAN_SYS_DIR/etc/systemd/system/* /etc/systemd/system
...
sed -i -e "s/#ZYNTHIAN_DIR#/$ZYNTHIAN_DIR_ESC/g" /etc/systemd/user/zynthian-webconf.service
sed -i -e "s/#ZYNTHIAN_CONFIG_DIR#/$ZYNTHIAN_CONFIG_DIR_ESC/g" /etc/systemd/user/zynthian-webconf.service
sed -i -e "s/#ZYNTHIAN_SYS_DIR#/$ZYNTHIAN_SYS_DIR_ESC/g" /etc/systemd/user/zynthian-webconf.service
# Check Kit Version service
sed -i -e "s/#ZYNTHIAN_SYS_DIR#/$ZYNTHIAN_SYS_DIR_ESC/g" /etc/systemd/system/check_kit_version.service
# Zynthbox Wifi Manager service
sed -i -e "s/#ZYNTHIAN_CONFIG_DIR#/$ZYNTHIAN_CONFIG_DIR_ESC/g" /etc/systemd/user/zynthbox-wifi-manager-start.service
...
# Midi Server Service
sed -i -e "s/#ZYNTHIAN_DIR#/$ZYNTHIAN_DIR_ESC/g" /etc/systemd/user/midi-server.service
sed -i -e "s/#ZYNTHIAN_CONFIG_DIR#/$ZYNTHIAN_CONFIG_DIR_ESC/g" /etc/systemd/user/midi-server.service
sed -i -e "s/#ZYNTHIAN_SYS_DIR#/$ZYNTHIAN_SYS_DIR_ESC/g" /etc/systemd/user/midi-server.service
...
systemctl disable --global pipewire-pulse.service
...
# Check if ZYNTHBOXOS_IMAGE_BUILD_DATETIME exists in os-release
